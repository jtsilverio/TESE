...
We tested for seasonal differences in mean daily VeDBA, time spent in each state and diurnality using ANOVA followed by post-hoc Tukey-Kramer's test. Daily Activity patterns were visualized using Gaussian kernel density estimates. All analysis were done in R using the base packages [@rcoreteam2020].
...
mean_time_in_state = time_in_state %>% 
  group_by(Season = season, State = state) %>% 
...
Circadian rhythms are more pronounced in the the high state 

```{r read-rhythmicity, message=FALSE, warning=FALSE, include=FALSE}
peaks_acf = readRDS("../03_analysis/ACF/peaks_acf.rds")
peaks_period = readRDS("../03_analysis/periodogram/peaks_period.rds")
```

```{r}
acf_summary = peaks_acf %>% 
  filter(state != "rest") %>% 
  group_by(Season = season, State = state) %>% 
  summarise("Total Animals" = length(rhythmic), "Rhythmic Animals" = sum(rhythmic), "Percentage" = sum(rhythmic)/length(rhythmic))
acf_summary
```


- tabela com numero de animais
  - qtd animais classificados como ritmicos
  - qtd de animais classificados como aritmicos
  - periodo medio

```{r acf-period-plot, fig.width=9, fig.asp=0.7,fig.cap = "Distribution"}
peaks_acf_plot = peaks_acf %>% filter(state != "rest")
peaks_period_plot = peaks_period %>% filter(state != "rest")

acf_dist_plot = ggplot(peaks_acf_plot, aes(x = season, y = acf, color = season)) +
...
period_dist_plot = ggplot(peaks_period_plot, aes(x = season, y = peak/60, color = season)) +
    facet_grid(~state) +
    gghalves::geom_half_boxplot() +
    gghalves::geom_half_point(transformation = position_jitter(width = 0.05, height = 0)) +
    egg::theme_article() +
...
acf_dist_plot / period_dist_plot
```
