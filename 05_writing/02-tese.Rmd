...
ggplot(tuco, aes(x = vedba)) +
    geom_histogram(data = tuco, 
                   ...
                   binwidth = 0.015, 
                   color = "white") +
    scale_colour_manual(name = "States", values = c(tuco_pal, Marginal = "grey40")) +
    #theme_article() +
    xlab("VeDBA") +
    ylab("Density") +
...
plot_season = ggplot(tuco[tuco$medium,]) +
    ...
    geom_histogram(aes(x = time, y = ..density..), binwidth = 60, alpha = 0.1, color = "white") +
    geom_density(aes(x = time), alpha = 0.8, bw = 60, kernel = "g", size = 0.7) +
    geom_rug(aes(x = time), alpha = 0.01) +
    facet_wrap(~season, ncol = 2) +
...
```{r diurnality-plot, fig.pos = "H", fig.width=9, fig.asp=0.4, out.width="95%", fig.cap = "Distribution of each state's diurnality index. Only the High Activity State is predominantly diurnal, with diurnality greater than 0.7 across all season."}
...
Circadian rhythms are present in all states as seen in the visual analysis of the autocorrelation plots (Table \@ref(tab:rhymicity-table); REF APPENDIX - plotar ACFs). The percentage of animals classified as rhythmic is higher for the high activity state, where a total of 18 animals were classified as rhythmic. In contrast, the medium activity state had, in total, 11 animals classified as rhythmic (Table \@ref(tab:rhymicity-table)).
...
```{r plot-ri-and-period, fig.width=8, fig.asp=0.8, fig.pos = "H", out.width="100%", fig.cap = "Distribution of Rhythmicity Index for state-labelled data and VeDBA. The distribution of the Medium State is statistically different from all other states and VeDBA. Graphs shows half boxplots and individual data"}
