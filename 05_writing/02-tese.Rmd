...
ts_vedba_zoom = ggplot(tuco_selected) +
    ...
    labs(color = "State") +
    guides(colour = guide_legend(override.aes = list(size=1.5))) +
    ggforce::facet_zoom(x = day_number == 2, zoom.size = 1, show.area = F) +
    theme_bw()
...
color = tuco_pal[1]
plot_id = ggplot(tuco %>% filter(state == "Rest")) +
    geom_histogram(aes(x = time, y = ..density..), 
                   binwidth = 60, 
                   alpha = 0.2,
                   fill = color) +
...
plot_season = ggplot(tuco %>% filter(state == "Rest")) +
...
color = tuco_pal[2]
plot_id = ggplot(tuco %>% filter(state == "Medium")) +
    geom_histogram(aes(x = time, y = ..density..), 
                   binwidth = 60, 
                   alpha = 0.2,
                   fill = color) +
...
plot_season = ggplot(tuco %>% filter(state == "Medium")) +
    geom_vline(data = sunriset_season, aes(xintercept = dawn), 
               linetype = "dotted", color = "grey70") +
    geom_vline(data = sunriset_season, aes(xintercept = dusk), 
               linetype = "dotted", color = "grey70") +
...
color = tuco_pal[3]
plot_id = ggplot(tuco %>% filter(state == "High")) +
...
plot_season = ggplot(tuco %>% filter(state == "High")) +
    geom_vline(data = sunriset_season, aes(xintercept = dawn),
               linetype = "dotted", color = "grey70") +
    geom_vline(data = sunriset_season, aes(xintercept = dusk),
               linetype = "dotted", color = "grey70") +
...
```{r aboveground-activity,  fig.asp=1, fig.width=9, out.width="100%", fig.pos="H", fig.align='center', fig.cap = "Distribution of the percentages of each activity state that happens aboveground during daytime."}
...
abvg_bar = ggplot(aboveground_activity, aes(perc, ID, fill = state, group = season)) +
    ...
    scale_x_continuous(labels = scales::percent_format()) +
    ylab("") +
    xlab("Mean percentage of aboveground activity in state")
...
plot_abvact_time = ggplot(aboveground_activity, aes(season, time, color = state)) +
    ...
    ylab("Mean time spent in\naboveground activity per animal (min)") +
    xlab("") +
