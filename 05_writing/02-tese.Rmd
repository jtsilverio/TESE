...
We labelled VeDBA data using the Viterbi algorithm [@zucchini2016]. With the state-labeled data we were able to dissociate and visualize the daily patterns of each different state. Actograms and time series plot shows how the different states are related to the calculated VeDBA (Fig. \@ref(fig:actograms-results)). Visual analysis of diel rhythms in VeDBA and in the state-labelled data indicates the daily rhythm is more robust in the High Activity state in comparison to Medium Activity.
...
```{r plot-patterns, fig.height = 15, fig.width=9, out.width="88%", fig.cap="Density estimate of daily activity patterns of tuco-tucos' behavioral states. Solid lines indicate the Gaussian kernel density estimates. Light-colored bars show observed distribution of each behavioral state occurrence. Rug lines above the x-axis shows individual occurrences. Dotted vertical lines show time of civil twilights. (A) High Activity State shows a diurnal pattern independent of the time of the year. (B) Medium Activity State shows no daily pattern. (C) Light Exposure shows a diurnal rhythm that changes according to the season."}
...
```{r diurnality-plot, fig.pos = "H", fig.width=9, fig.asp=0.4, out.width="95%", fig.cap = "Distribution of each state's diurnality index. Only the High Activity State is predominantly diurnal. High Activity State had a average diurnality greater than 70\\% acriss all season."}
...
kable(mean_diurnality, caption = "Mean diurnality of each state.")
```
...
acf_peaks_vedba = acf_peaks %>% filter(state == "vedba")
period_peaks_vedba = period_peaks %>% filter(state == "vedba")

#acf_peaks = acf_peaks %>% filter(state != "Rest")
#period_peaks = period_peaks %>% filter(state != "Rest")
```
...
We also estimated RI and periodicity for VeDBA. In comparison with state-labelled data VeDBA analysis are more similar to the high activity state. The number of animals classified as rhythmic based on the autocorrelation plot is similar between VeDBA and high activity state data (Table \@ref(tab:rhymicity-table)). The distribution of RI and the period estimation using VeDBA does not visually differ greatly (Figure \@ref(fig:plot-ri-and-period) A and B). For VeDBA the mean estimated period was `r period_mean[period_mean$state == "vedba",]$mean_sd`. \newline

```{r plot-ri-and-period, fig.width=7, fig.asp=0.9, fig.pos = "H", out.width="100%", fig.cap = "Distribution of Rhythmicity Index for state-labelled data and VeDBA."}
