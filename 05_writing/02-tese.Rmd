...
```{r methods-map, echo=F, out.width = "100%", fig.cap="Study site location (orange polygon) at the Monte Desert, approximately 5km away from the village of Anillaco, northwest of Argentina. Study site has a area of approximately 75 m². Geographical information retrieved from ESRI satellites (Datum SIRGAS 2002 UTM 19s)."}
...
Light exposure was used to analyse the time tuco-tucos spend on the surface and to further classify VeDBA data points as above or below ground. The threshold for considering a data point as being aboveground was 2 lux, consistent with what has been done in @jannetti2019. Accelerometer and lightlogger data were merged accordingly to the date and times of recordings using purposely written R scripts [@rcoreteam2020]. Time of recordings between both devices were not synchronized to the minute. Consequently, we had to round lightlogger recording times to the nearest 5 minutes to merge both data streams.
...
### Time on Surface & General Activity 
...
time_aboveground = tuco %>% 
	...
	summarise(daylength = sum(daytime),
	          time_aboveground = sum(aboveground, na.rm = T),
...
Tuco-tucos spent an overall mean and standard deviation of `r time_aboveground %>% summarise(mean = mean(mean_t_aboveground), sd = sd(mean_t_aboveground)) %>% summarise(mean = paste0(round(mean,2)," ± ", round(sd, 2)," minutes"))` of the day on the surface. This corresponds to a mean percentage of the day length of `r time_aboveground %>% filter(season == "July") %>% summarise(mean = mean(mean_p_aboveground), sd = sd(mean_p_aboveground)) %>% summarise(mean = paste0(round(mean,2), " ± ", round(sd, 2)))` in July and `r time_aboveground %>% filter(season == "February") %>% summarise(mean = mean(mean_p_aboveground), sd = sd(mean_p_aboveground)) %>% summarise(mean = paste0(round(mean,2), " ± ", round(sd, 2)))` in February. Time on surface shows no significant annual changes. The percentage of the day length spent on the surface, however, was significantly different between July and February (ANOVA; F = 4.39, p < 0.05). \newline
...
plot_id = ggplot(tuco[tuco$aboveground]) +
    ...
    geom_density(aes(x = time), 
                 ...
                 kernel = "g",
                 size = 0.5) +
    facet_wrap(~factor(ID,levels = ID, labels = paste0("ID:", ID)),
               ncol = 3) +
...
The daytime VeDBA (Light Phase Average) is also significantly different between months (ANOVA; F = 7.282, p < 0.001). Post hoc comparisons using Tukey-Kramer's Test shows a difference of 0.035g between the means of October and July (p < 0.05). \newline
...
bx_daytime_vedba = ggplot(data = vedba_daytime,
    ...
    gghalves::geom_half_boxplot(nudge = 0.05,
                              outlier.color = NA) +
    scale_x_discrete(labels = c("Mar","Jul","Oct","Feb")) +
    scale_y_continuous(breaks = c(0.10, 0.13, 0.16, 0.19, 0.22, 0.25)) +
    xlab("") +
    ylab("Mean Daytime \nGeneral Activity (VeDBA)") +
    theme(legend.position = "none")


df1 = data.frame(x = c(1.9, 3), y = c(0.22,0.22))
bx_daytime_vedba = bx_daytime_vedba +
   geom_line(data = df1, mapping = aes(x,y, group = 1), color = "grey60") +
   annotate("text", x = 2.5, y = 0.222, label = "*", size = 4, color = "grey60")
...
daily_budget_id = tuco %>%
    ...
    summarise(Rest   = sum(state == "Rest")/n(),
...
daily_budget_id$ID = paste0("ID:", daily_budget_id$ID)
...
plot_abvact_perc = ggplot(aboveground_activity, aes(state, perc, color = state)) +
    ...
    scale_color_manual(values = tuco_pal[1:3]) +
    theme(legend.position = "none") +
    scale_y_continuous(labels = scales::percent_format(), limits = c(0,0.10))
