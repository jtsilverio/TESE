...
# Model 06: ~ season + time --------------------------------------------------------
formula = ~ season + cosinor(time, period = 24)
par0_m6 = getPar0(model = m3, formula = formula)
m6 %<-% fitHMM(data = tuco,
               formula = formula,
               nbStates = 3,
               dist = list(vedba = "gamma"),
...
# Model 07: ~ sex * time --------------------------------------------------------
formula = ~ sex * cosinor(time, period = 24)
par0_m7 = getPar0(model = m2, formula = formula)
m7 %<-% fitHMM(data = tuco,
               formula = formula,
               nbStates = 3,
               dist = list(vedba = "gamma"),
...
par0_m11 = getPar0(model = m4, formula = formula)
...
formula = ~(season + sex) * cosinor(time, period = 24)
par0_m12 = getPar0(model = m11, formula = formula)
...
formula = ~season * sex + cosinor(time, period = 24)
par0_m13 = getPar0(model = m10, formula = formula)
...
# Model 14: ~season * time + sex ------------------------------------------
formula = ~season * cosinor(time, period = 24) + sex
par0_m14 = getPar0(model = m8, formula = formula)
...
# Model 15: ~season * sex * time ------------------------------------------
formula = ~season * sex * cosinor(time, period = 24)
par0_m15 = getPar0(model = m11, formula = formula)
m15 %<-% fitHMM(data = tuco,
                formula = formula,
                nbStates = 3,
                dist = list(vedba = "gamma"),
                Par0 = par0_m15$Par)
...
message("Waiting for m11, m12, m13 and m14, m15 to be resolved ...\n")
while (!(resolved(f_m11) & resolved(f_m12) & resolved(f_m13) & resolved(f_m14) & resolved(f_m15))) {
    Sys.sleep(0.2)
}
message("\n\n\n")
Sys.time() - start_time
invisible(resolve(list(m11, m12, m13, m14, m15)))
