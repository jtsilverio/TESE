...
# range = tuco %>%
#   group_by(ID) %>% 
#   summarise(Max = max(vedba), Min = min(vedba), Range = Max - Min)
# 
# kable(range, caption = "Range dos valores de VeDBA por animal.")
```
...
hist_data = tuco %>% filter(day_number < 5 & day_number > 1)

graph_hist = ggplot(hist_data) +
...
sexlabels = unique(hist_data %>% select(ID, sex) %>% mutate(sex = if_else(sex == "m", "♂", "♀"))) 
...
ggplot(hist_data) +
  ggridges::geom_density_ridges(aes(x = vedba, y = ID, fill = season, height = ..density..), color = "grey40", alpha = 0.9, stat = "density", trim = T) +
